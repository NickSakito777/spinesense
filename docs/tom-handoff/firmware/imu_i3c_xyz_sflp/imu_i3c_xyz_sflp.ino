#include <Arduino.h>
#include <math.h>

extern "C" {
#ifndef HAL_I3C_MODULE_ENABLED
#define HAL_I3C_MODULE_ENABLED
#endif
#include "stm32u3xx_hal.h"
#include "stm32u3xx_hal_i3c.h"
#include "stm32u3xx_hal_pwr_ex.h"
#include "stm32u3xx_ll_i3c.h"
}

// ============================================================================
// ISM6HG256X I3C 5-IMU streaming, raw accel/gyro + SFLP game-rotation quaternion.
//
// This is an ADDITIVE variant of imu_i3c_xyz.ino: the raw accel/gyro path is
// byte-for-byte identical (same OUTX_L_G burst, same six columns). It ALSO turns
// on the on-chip SFLP (Sensor Fusion Low Power) game rotation vector and appends
// four quaternion columns (qw qx qy qz) to the END of each line, so older raw-only
// parsers that read columns 0..9 keep working unchanged.
//
// SFLP read method: the ISM6HG256X exposes the full quaternion (W,X,Y,Z) directly
// in embedded-function-bank registers SFLP_QUATW_L..SFLP_QUATZ_H (0x2A..0x31), each
// IEEE half-float. No FIFO needed. Registers/bits below are from ST's official
// ism6hg256x-pid driver. If SFLP misbehaves, flash imu_i3c_xyz.ino (raw-only) back.
// ============================================================================

HardwareSerial VCP(PA10_ALT1, PA9_ALT1);  // RX, TX: USART1 -> ST-LINK VCP

static I3C_HandleTypeDef hi3c2;

static constexpr uint8_t IMU_COUNT = 5;
static constexpr uint8_t IMU_ASSIGN_STATIC_ADDR = 0x6B;  // TA0/SA0 high
static constexpr uint32_t STREAM_BAUD = 921600;
static constexpr uint32_t STREAM_INTERVAL_US = 8333;      // target ~120 Hz frames; actual rate is logged/checked offline

static constexpr uint8_t Broadcast_DISEC = 0x01;
static constexpr uint8_t Broadcast_RSTDAA = 0x06;
static constexpr uint8_t Direct_SETDASA = 0x87;

// DAA robustness for long-yarn/garment bus (2026-08 field patch):
// DAA runs on a slow open-drain-friendly bus profile, then the bus is
// re-initialised at the original streaming profile once addresses stick.
static constexpr uint32_t BOOT_SETTLE_MS = 150;
static constexpr uint8_t RSTDAA_RETRIES = 3;
static constexpr uint8_t SETDASA_RETRIES = 3;
static constexpr uint32_t RETRY_GAP_MS = 30;

// Runtime self-healing (2026-08 field patch): a transient glitch on the shared
// lines resets the sensors and drops their dynamic addresses; without this the
// rest of the session is READ_FAIL. Re-enumerate + reconfigure automatically.
static constexpr uint16_t BUS_FAIL_FRAMES = 12;      // every live IMU failing ~0.1 s
static constexpr uint16_t DEV_FAIL_FRAMES = 60;      // one live IMU failing ~0.5 s
static constexpr uint32_t RECOVER_MIN_GAP_MS = 1500;
static constexpr uint32_t RECOVER_MAX_GAP_MS = 10000;

static constexpr uint8_t REG_WHO_AM_I = 0x0F;
static constexpr uint8_t REG_FUNC_CFG_ACCESS = 0x01;  // bit7 (0x80) = embedded-function bank access
static constexpr uint8_t REG_CTRL1 = 0x10;
static constexpr uint8_t REG_CTRL2 = 0x11;
static constexpr uint8_t REG_CTRL3 = 0x12;
static constexpr uint8_t REG_CTRL6 = 0x15;
static constexpr uint8_t REG_CTRL8 = 0x17;
static constexpr uint8_t REG_OUTX_L_G = 0x22;

// Embedded-function-bank registers (only valid after FUNC_CFG_ACCESS = 0x80).
static constexpr uint8_t REG_EMB_FUNC_EN_A = 0x04;     // bit1 (0x02) = sflp_game_en
static constexpr uint8_t REG_EMB_SFLP_ODR = 0x5E;      // bits[5:3] = sflp_game_odr
static constexpr uint8_t REG_EMB_FUNC_INIT_A = 0x66;   // bit1 (0x02) = sflp_game_init
static constexpr uint8_t REG_EMB_SFLP_QUATW_L = 0x2A;  // W_L,W_H,X_L,X_H,Y_L,Y_H,Z_L,Z_H (8 bytes, half-float LE)

static constexpr uint8_t SFLP_ODR_120HZ_CODE = 0x3;    // ism6hg256x_sflp_data_rate_t: 120 Hz

static constexpr uint8_t WHO_EXPECTED = 0x73;

struct ImuSlot {
  const char *label;
  const char *position;
  const char *ta0Name;
  GPIO_TypeDef *ta0Port;
  uint16_t ta0Pin;
  uint8_t dynAddr;
  bool ok;          // configured + streaming
  bool addrOk;      // SETDASA succeeded in the last enumeration
  uint16_t failStreak;
};

static ImuSlot imus[IMU_COUNT] = {
  {"IMU0", "POSITION_0_EXTRA", "D2/PC8", GPIOC, GPIO_PIN_8, 0x32, false},
  {"IMU1", "PARENT_BOTTOM_FIXED", "D4/PB5", GPIOB, GPIO_PIN_5, 0x33, false},
  // 2026-08-19: IMU2 TA0 moved D7/PA8 -> D6/PB10 (D7 path suspected dead; see
  // connectivity_5 fault isolation). D6/PB10 per NUCLEO_U385RG_Q variant map.
  {"IMU2", "CHILD_TOP_MOVING", "D6/PB10", GPIOB, GPIO_PIN_10, 0x34, false},
  {"IMU3", "POSITION_3_EXTRA", "D8/PC7", GPIOC, GPIO_PIN_7, 0x35, false},
  {"IMU4", "POSITION_4_EXTRA", "D9/PC6", GPIOC, GPIO_PIN_6, 0x36, false},
};

static volatile bool i3cDone = false;
static volatile bool i3cError = false;

static uint32_t ctrlBuf[24];
static uint8_t txBuf[64];
static uint8_t rxBuf[32];

static void printHex8(uint8_t value);
static void printPositionMap();
static void printTableHeader();
static void initTa0Gpios();
static void selectOnlyTa0(uint8_t index);
static void setAllTa0Low();
static bool waitI3CReady(uint32_t timeoutMs);
static bool initI3C2(bool slowDaaProfile);
static void recoverI3CIfNeeded();
static bool resetDynamicAddresses();
static bool setDynamicAddress(ImuSlot &imu);
static bool assignDynamicAddresses();
static uint8_t configureAllImus();
static void recoverBus(uint32_t now);
static bool i3cWriteReg(uint8_t dynAddr, uint8_t reg, uint8_t value);
static bool i3cReadReg(uint8_t dynAddr, uint8_t reg, uint8_t *data, uint16_t len);
static bool initISM6HG256X(ImuSlot &imu);
static bool enableSflp(uint8_t dynAddr);
static bool readImuBurst(uint8_t dynAddr, int16_t &gx, int16_t &gy, int16_t &gz, int16_t &ax, int16_t &ay, int16_t &az);
static bool readSflpQuat(uint8_t dynAddr, float &qw, float &qx, float &qy, float &qz);
static float halfToFloat(uint16_t h);

extern "C" void HAL_I3C_MspInit(I3C_HandleTypeDef *hi3c)
{
  if (hi3c->Instance != I3C2) {
    return;
  }

  __HAL_RCC_PWR_CLK_ENABLE();
  HAL_PWREx_EnablePullUpPullDownConfig();
  HAL_PWREx_EnableI3CPullUp(PWR_I3CPU_PB13);  // PB13 / D15 / I3C2_SCL
  HAL_PWREx_EnableI3CPullUp(PWR_I3CPU_PB14);  // PB14 / D14 / I3C2_SDA

  RCC_PeriphCLKInitTypeDef clk = {};
  clk.PeriphClockSelection = RCC_PERIPHCLK_I3C2;
  clk.I3c2ClockSelection = RCC_I3C2CLKSOURCE_PCLK2;
  HAL_RCCEx_PeriphCLKConfig(&clk);

  __HAL_RCC_I3C2_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();

  GPIO_InitTypeDef gpio = {};
  gpio.Pin = GPIO_PIN_13 | GPIO_PIN_14;
  gpio.Mode = GPIO_MODE_AF_PP;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  gpio.Alternate = GPIO_AF6_I3C2;
  HAL_GPIO_Init(GPIOB, &gpio);

  HAL_NVIC_SetPriority(I3C2_EV_IRQn, 0, 0);
  HAL_NVIC_EnableIRQ(I3C2_EV_IRQn);
  HAL_NVIC_SetPriority(I3C2_ER_IRQn, 0, 0);
  HAL_NVIC_EnableIRQ(I3C2_ER_IRQn);

  HAL_Delay(1);
}

extern "C" void I3C2_EV_IRQHandler(void)
{
  HAL_I3C_EV_IRQHandler(&hi3c2);
}

extern "C" void I3C2_ER_IRQHandler(void)
{
  HAL_I3C_ER_IRQHandler(&hi3c2);
}

extern "C" void HAL_I3C_CtrlTxCpltCallback(I3C_HandleTypeDef *hi3c)
{
  if (hi3c->Instance == I3C2) {
    i3cDone = true;
  }
}

extern "C" void HAL_I3C_CtrlRxCpltCallback(I3C_HandleTypeDef *hi3c)
{
  if (hi3c->Instance == I3C2) {
    i3cDone = true;
  }
}

extern "C" void HAL_I3C_CtrlMultipleXferCpltCallback(I3C_HandleTypeDef *hi3c)
{
  if (hi3c->Instance == I3C2) {
    i3cDone = true;
  }
}

extern "C" void HAL_I3C_ErrorCallback(I3C_HandleTypeDef *hi3c)
{
  if (hi3c->Instance == I3C2) {
    i3cError = true;
    i3cDone = true;
  }
}

void setup()
{
  VCP.begin(STREAM_BAUD);
  delay(300);
  VCP.println();
  VCP.println("ISM6HG256X I3C 5-IMU xyz+SFLP test");
  printPositionMap();

  initTa0Gpios();
  VCP.println("TA0 GPIO init OK");

  if (!initI3C2(true)) {  // slow open-drain-friendly profile for DAA
    VCP.println("ERR: I3C2 init failed");
    return;
  }
  VCP.println("I3C2 init OK (slow DAA profile)");

  // Let sensor VDD settle and on-chip boot finish even behind a resistive
  // contact (slow supply ramp) before the first DAA.
  delay(BOOT_SETTLE_MS);

  if (!assignDynamicAddresses()) {
    VCP.println("ERR: SETDASA failed. Check TA0 wires, SDA/SCL, pull-ups, power, and CS high.");
    return;
  }

  // Addresses live in the sensors; re-init the controller at streaming speed.
  HAL_I3C_DeInit(&hi3c2);
  delay(2);
  if (!initI3C2(false)) {
    VCP.println("ERR: I3C2 re-init at streaming profile failed");
    return;
  }
  VCP.println("I3C2 switched to streaming profile");

  const uint8_t okCount = configureAllImus();
  VCP.print("Config OK ");
  VCP.print(okCount);
  VCP.print("/");
  VCP.print(IMU_COUNT);
  VCP.println(" (raw + SFLP game rotation vector); self-healing armed");
  printTableHeader();
}

void loop()
{
  static uint32_t lastHeaderMs = 0;
  static uint32_t nextFrameUs = 0;
  const uint32_t nowUs = micros();
  if ((int32_t)(nowUs - nextFrameUs) < 0) {
    return;
  }
  if ((uint32_t)(nowUs - nextFrameUs) > STREAM_INTERVAL_US * 4UL) {
    nextFrameUs = nowUs;
  } else {
    nextFrameUs += STREAM_INTERVAL_US;
  }

  const uint32_t now = millis();

  if (now - lastHeaderMs > 5000) {
    printTableHeader();
    lastHeaderMs = now;
  }

  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    int16_t gxRaw = 0, gyRaw = 0, gzRaw = 0;
    int16_t axRaw = 0, ayRaw = 0, azRaw = 0;

    if (!readImuBurst(imus[i].dynAddr, gxRaw, gyRaw, gzRaw, axRaw, ayRaw, azRaw)) {
      VCP.print(now);
      VCP.print('\t');
      VCP.print(imus[i].label);
      VCP.print('\t');
      VCP.print(imus[i].position);
      VCP.println("\tREAD_FAIL");
      if (imus[i].failStreak < 0xFFFF) {
        imus[i].failStreak++;
      }
      recoverI3CIfNeeded();   // never let the HAL sit in an error state
      continue;
    }
    imus[i].failStreak = 0;

    // SFLP quaternion from the embedded-function bank. Raw is the hard requirement, so a quaternion
    // read failure does not drop the line: emit nan so the column structure stays 14-wide.
    float qw = NAN, qx = NAN, qy = NAN, qz = NAN;
    readSflpQuat(imus[i].dynAddr, qw, qx, qy, qz);

    const float axMg = axRaw * 0.488f;            // low-g accel FS = +/-16 g
    const float ayMg = ayRaw * 0.488f;
    const float azMg = azRaw * 0.488f;
    const float gxDps = gxRaw * 70.0f / 1000.0f;  // gyro FS = +/-2000 dps
    const float gyDps = gyRaw * 70.0f / 1000.0f;
    const float gzDps = gzRaw * 70.0f / 1000.0f;

    VCP.print(now);
    VCP.print('\t');
    VCP.print(imus[i].label);
    VCP.print('\t');
    VCP.print(imus[i].position);
    VCP.print('\t');
    VCP.print("0x");
    printHex8(imus[i].dynAddr);
    VCP.print('\t');
    VCP.print(axMg, 1);
    VCP.print('\t');
    VCP.print(ayMg, 1);
    VCP.print('\t');
    VCP.print(azMg, 1);
    VCP.print('\t');
    VCP.print(gxDps, 2);
    VCP.print('\t');
    VCP.print(gyDps, 2);
    VCP.print('\t');
    VCP.print(gzDps, 2);
    VCP.print('\t');
    VCP.print(qw, 5);
    VCP.print('\t');
    VCP.print(qx, 5);
    VCP.print('\t');
    VCP.print(qy, 5);
    VCP.print('\t');
    VCP.println(qz, 5);
  }

  // Self-healing decision: bus-level loss (every live IMU failing) or a single
  // previously-good IMU stuck failing -> re-enumerate (rate-limited inside).
  uint8_t live = 0, liveFailing = 0;
  bool devStuck = false;
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    if (!imus[i].ok) {
      continue;
    }
    live++;
    if (imus[i].failStreak >= BUS_FAIL_FRAMES) {
      liveFailing++;
    }
    if (imus[i].failStreak >= DEV_FAIL_FRAMES) {
      devStuck = true;
    }
  }
  const bool busLost = (live == 0) || (liveFailing == live);
  if (busLost || devStuck) {
    recoverBus(now);
  }

  // No fixed delay here: the frame scheduler above controls the target stream rate. Adding the SFLP
  // read (one bank-switch + 8-byte read + bank-restore per IMU) lowers the achievable rate; that is
  // fine -- the analysis uses the MEASURED median rate, not the 120 Hz target.
}

static void printHex8(uint8_t value)
{
  if (value < 0x10) {
    VCP.print('0');
  }
  VCP.print(value, HEX);
}

static void printPositionMap()
{
  VCP.println("slot\tposition\tTA0_pin\tdyn_addr");
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    VCP.print(imus[i].label);
    VCP.print('\t');
    VCP.print(imus[i].position);
    VCP.print('\t');
    VCP.print(imus[i].ta0Name);
    VCP.print('\t');
    VCP.print("0x");
    printHex8(imus[i].dynAddr);
    VCP.println();
  }
  VCP.println();
}

static void printTableHeader()
{
  VCP.println("t_ms\timu\tposition\taddr\tax_mg\tay_mg\taz_mg\tgx_dps\tgy_dps\tgz_dps\tqw\tqx\tqy\tqz");
}

static void initTa0Gpios()
{
  __HAL_RCC_GPIOA_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();
  __HAL_RCC_GPIOC_CLK_ENABLE();

  GPIO_InitTypeDef gpio = {};
  gpio.Mode = GPIO_MODE_OUTPUT_PP;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_LOW;

  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    gpio.Pin = imus[i].ta0Pin;
    HAL_GPIO_Init(imus[i].ta0Port, &gpio);
  }

  setAllTa0Low();
}

static void selectOnlyTa0(uint8_t index)
{
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    HAL_GPIO_WritePin(imus[i].ta0Port, imus[i].ta0Pin, (i == index) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  }
  delay(2);
}

static void setAllTa0Low()
{
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    HAL_GPIO_WritePin(imus[i].ta0Port, imus[i].ta0Pin, GPIO_PIN_RESET);
  }
  delay(2);
}

static bool waitI3CReady(uint32_t timeoutMs)
{
  const uint32_t start = millis();
  while (!i3cDone && !i3cError && (millis() - start < timeoutMs)) {
    yield();
  }
  return i3cDone && !i3cError && (HAL_I3C_GetState(&hi3c2) == HAL_I3C_STATE_READY);
}

static bool currentProfileSlow = false;

static bool initI3C2(bool slowDaaProfile)
{
  I3C_FifoConfTypeDef fifo = {};
  I3C_CtrlConfTypeDef ctrl = {};

  currentProfileSlow = slowDaaProfile;
  hi3c2.Instance = I3C2;
  hi3c2.Mode = HAL_I3C_MODE_CONTROLLER;
  hi3c2.Init.CtrlBusCharacteristic.WaitTime = HAL_I3C_OWN_ACTIVITY_STATE_0;
  hi3c2.Init.CtrlBusCharacteristic.SCLI2CHighDuration = 0x00;
  if (slowDaaProfile) {
    // ~4x slower clock, longest open-drain low, extra SDA hold: survives the
    // high-RC garment bus during the electrically fragile DAA phase.
    hi3c2.Init.CtrlBusCharacteristic.SDAHoldTime = HAL_I3C_SDA_HOLD_TIME_1_5;
    hi3c2.Init.CtrlBusCharacteristic.SCLPPLowDuration = 0xBF;
    hi3c2.Init.CtrlBusCharacteristic.SCLI3CHighDuration = 0xBF;
    hi3c2.Init.CtrlBusCharacteristic.SCLODLowDuration = 0xFF;
    hi3c2.Init.CtrlBusCharacteristic.BusFreeDuration = 0x4C;
    hi3c2.Init.CtrlBusCharacteristic.BusIdleDuration = 0xFF;
  } else {
    // Original streaming profile (unchanged: 120 Hz five-stream throughput).
    hi3c2.Init.CtrlBusCharacteristic.SDAHoldTime = HAL_I3C_SDA_HOLD_TIME_0_5;
    hi3c2.Init.CtrlBusCharacteristic.SCLPPLowDuration = 0x2F;
    hi3c2.Init.CtrlBusCharacteristic.SCLI3CHighDuration = 0x2F;
    hi3c2.Init.CtrlBusCharacteristic.SCLODLowDuration = 0x2F;
    hi3c2.Init.CtrlBusCharacteristic.BusFreeDuration = 0x13;
    hi3c2.Init.CtrlBusCharacteristic.BusIdleDuration = 0x5E;
  }

  if (HAL_I3C_Init(&hi3c2) != HAL_OK) {
    return false;
  }

  fifo.RxFifoThreshold = HAL_I3C_RXFIFO_THRESHOLD_4_4;
  fifo.TxFifoThreshold = HAL_I3C_TXFIFO_THRESHOLD_1_4;
  fifo.ControlFifo = HAL_I3C_CONTROLFIFO_DISABLE;
  fifo.StatusFifo = HAL_I3C_STATUSFIFO_DISABLE;
  if (HAL_I3C_SetConfigFifo(&hi3c2, &fifo) != HAL_OK) {
    return false;
  }

  ctrl.DynamicAddr = 0;
  ctrl.StallTime = 0;
  ctrl.HotJoinAllowed = DISABLE;
  ctrl.ACKStallState = DISABLE;
  ctrl.CCCStallState = DISABLE;
  ctrl.TxStallState = DISABLE;
  ctrl.RxStallState = DISABLE;
  ctrl.HighKeeperSDA = DISABLE;
  return HAL_I3C_Ctrl_Config(&hi3c2, &ctrl) == HAL_OK;
}

// After a NACK/timeout on a marginal bus the HAL can be left in an error
// state that fails every later transfer. Re-init on the current profile.
static void recoverI3CIfNeeded()
{
  if (HAL_I3C_GetState(&hi3c2) == HAL_I3C_STATE_READY) {
    return;
  }
  HAL_I3C_DeInit(&hi3c2);
  delay(2);
  if (!initI3C2(currentProfileSlow)) {
    VCP.println("ERR: I3C2 recover failed");
  }
}

static bool resetDynamicAddresses()
{
  uint8_t disecPayload = 0x08;
  I3C_CCCTypeDef broadcastCcc[] = {
    {0, Broadcast_DISEC, {&disecPayload, 1}, LL_I3C_DIRECTION_WRITE},
    {0, Broadcast_RSTDAA, {NULL, 0}, LL_I3C_DIRECTION_WRITE},
  };

  I3C_XferTypeDef xfer = {};
  memset(ctrlBuf, 0, sizeof(ctrlBuf));
  memset(txBuf, 0, sizeof(txBuf));
  xfer.CtrlBuf.pBuffer = ctrlBuf;
  xfer.CtrlBuf.Size = 16;
  xfer.TxBuf.pBuffer = txBuf;
  xfer.TxBuf.Size = 1;

  if (HAL_I3C_AddDescToFrame(&hi3c2, broadcastCcc, NULL, &xfer, 2, I3C_BROADCAST_WITHOUT_DEFBYTE_RESTART) != HAL_OK) {
    return false;
  }

  i3cDone = false;
  i3cError = false;
  if (HAL_I3C_Ctrl_TransmitCCC_IT(&hi3c2, &xfer) != HAL_OK) {
    return false;
  }
  return waitI3CReady(1000);
}

static bool setDynamicAddress(ImuSlot &imu)
{
  uint8_t setdasaPayload = imu.dynAddr << 1;
  I3C_CCCTypeDef directCcc[] = {
    {IMU_ASSIGN_STATIC_ADDR, Direct_SETDASA, {&setdasaPayload, 1}, LL_I3C_DIRECTION_WRITE},
  };

  I3C_XferTypeDef xfer = {};
  memset(ctrlBuf, 0, sizeof(ctrlBuf));
  memset(txBuf, 0, sizeof(txBuf));
  xfer.CtrlBuf.pBuffer = ctrlBuf;
  xfer.CtrlBuf.Size = 16;
  xfer.TxBuf.pBuffer = txBuf;
  xfer.TxBuf.Size = 1;

  if (HAL_I3C_AddDescToFrame(&hi3c2, directCcc, NULL, &xfer, 1, I3C_DIRECT_WITHOUT_DEFBYTE_STOP) != HAL_OK) {
    return false;
  }

  i3cDone = false;
  i3cError = false;
  if (HAL_I3C_Ctrl_TransmitCCC_IT(&hi3c2, &xfer) != HAL_OK) {
    return false;
  }
  if (!waitI3CReady(1000)) {
    return false;
  }

  return HAL_I3C_Ctrl_IsDeviceI3C_Ready(&hi3c2, imu.dynAddr, 300, 1000) == HAL_OK;
}

static bool assignDynamicAddresses()
{
  setAllTa0Low();

  bool rstOk = false;
  for (uint8_t attempt = 1; attempt <= RSTDAA_RETRIES; attempt++) {
    if (resetDynamicAddresses()) {
      rstOk = true;
      break;
    }
    recoverI3CIfNeeded();
    delay(RETRY_GAP_MS);
  }
  if (!rstOk) {
    VCP.print("ERR: broadcast DISEC/RSTDAA failed after ");
    VCP.print(RSTDAA_RETRIES);
    VCP.println(" tries (shared bus: SCL/SDA/3V3/GND/main connector)");
    return false;
  }

  // Scan every chip even after a failure so the log names ALL bad branches.
  uint8_t failCount = 0;
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    selectOnlyTa0(i);

    VCP.print("SETDASA ");
    VCP.print(imus[i].label);
    VCP.print(" via ");
    VCP.print(imus[i].ta0Name);
    VCP.print(": static 0x6B -> dyn 0x");
    printHex8(imus[i].dynAddr);
    VCP.print(" ... ");

    bool assigned = false;
    for (uint8_t attempt = 1; attempt <= SETDASA_RETRIES; attempt++) {
      if (setDynamicAddress(imus[i])) {
        assigned = true;
        if (attempt > 1) {
          VCP.print("(try ");
          VCP.print(attempt);
          VCP.print(") ");
        }
        break;
      }
      recoverI3CIfNeeded();
      delay(RETRY_GAP_MS);
      selectOnlyTa0(i);
    }

    imus[i].addrOk = assigned;
    if (assigned) {
      VCP.println("OK");
    } else {
      VCP.print("FAIL after ");
      VCP.print(SETDASA_RETRIES);
      VCP.println(" tries");
      failCount++;
    }
    setAllTa0Low();
  }

  VCP.print("SETDASA summary: ");
  VCP.print(IMU_COUNT - failCount);
  VCP.print("/");
  VCP.print(IMU_COUNT);
  VCP.println(" assigned");
  return failCount < IMU_COUNT;   // stream with whatever enumerated
}

// Configure every IMU that holds an address. Returns how many are streaming-ready.
static uint8_t configureAllImus()
{
  uint8_t okCount = 0;
  for (uint8_t i = 0; i < IMU_COUNT; i++) {
    imus[i].failStreak = 0;
    imus[i].ok = imus[i].addrOk && initISM6HG256X(imus[i]);
    if (imus[i].ok) {
      okCount++;
    } else {
      VCP.print("WARN: ");
      VCP.print(imus[i].label);
      VCP.println(" not streaming (no address or init failed)");
    }
  }
  return okCount;
}

// Full bus recovery: slow-profile re-enumeration, back to streaming profile, reconfigure.
static void recoverBus(uint32_t now)
{
  static uint32_t gapMs = RECOVER_MIN_GAP_MS;
  static uint32_t lastMs = 0;
  if (lastMs != 0 && (now - lastMs) < gapMs) {
    return;
  }
  lastMs = now;
  VCP.print(now);
  VCP.println("\tRECOVER\tbegin");

  HAL_I3C_DeInit(&hi3c2);
  delay(2);
  if (!initI3C2(true)) {
    VCP.println("RECOVER: slow-profile init failed");
    return;
  }
  delay(BOOT_SETTLE_MS);
  assignDynamicAddresses();
  HAL_I3C_DeInit(&hi3c2);
  delay(2);
  if (!initI3C2(false)) {
    VCP.println("RECOVER: streaming-profile init failed");
    return;
  }
  const uint8_t okCount = configureAllImus();
  VCP.print(now);
  VCP.print("\tRECOVER\tdone ");
  VCP.print(okCount);
  VCP.print("/");
  VCP.println(IMU_COUNT);
  // back off while the garment stays dead; reset once anything streams
  gapMs = (okCount == 0) ? ((gapMs * 2 > RECOVER_MAX_GAP_MS) ? RECOVER_MAX_GAP_MS : gapMs * 2) : RECOVER_MIN_GAP_MS;
}

static bool i3cWriteReg(uint8_t dynAddr, uint8_t reg, uint8_t value)
{
  uint8_t payload[2] = {reg, value};
  I3C_PrivateTypeDef desc = {dynAddr, {payload, sizeof(payload)}, {NULL, 0}, HAL_I3C_DIRECTION_WRITE};
  I3C_XferTypeDef xfer = {};

  memset(ctrlBuf, 0, sizeof(ctrlBuf));
  memset(txBuf, 0, sizeof(txBuf));
  xfer.CtrlBuf.pBuffer = ctrlBuf;
  xfer.CtrlBuf.Size = 1;
  xfer.TxBuf.pBuffer = txBuf;
  xfer.TxBuf.Size = sizeof(payload);

  if (HAL_I3C_AddDescToFrame(&hi3c2, NULL, &desc, &xfer, 1, I3C_PRIVATE_WITH_ARB_STOP) != HAL_OK) {
    return false;
  }

  i3cDone = false;
  i3cError = false;
  if (HAL_I3C_Ctrl_Transmit_IT(&hi3c2, &xfer) != HAL_OK) {
    return false;
  }
  return waitI3CReady(1000);
}

static bool i3cReadReg(uint8_t dynAddr, uint8_t reg, uint8_t *data, uint16_t len)
{
  I3C_PrivateTypeDef desc[2] = {
    {dynAddr, {&reg, 1}, {NULL, 0}, HAL_I3C_DIRECTION_WRITE},
    {dynAddr, {NULL, 0}, {data, len}, HAL_I3C_DIRECTION_READ},
  };
  I3C_XferTypeDef xfer = {};

  memset(ctrlBuf, 0, sizeof(ctrlBuf));
  memset(txBuf, 0, sizeof(txBuf));
  xfer.CtrlBuf.pBuffer = ctrlBuf;
  xfer.CtrlBuf.Size = 2;
  xfer.TxBuf.pBuffer = txBuf;
  xfer.TxBuf.Size = 1;
  xfer.RxBuf.pBuffer = data;
  xfer.RxBuf.Size = len;

  if (HAL_I3C_AddDescToFrame(&hi3c2, NULL, desc, &xfer, 2, I3C_PRIVATE_WITH_ARB_RESTART) != HAL_OK) {
    return false;
  }

  i3cDone = false;
  i3cError = false;
  if (HAL_I3C_Ctrl_MultipleTransfer_IT(&hi3c2, &xfer) != HAL_OK) {
    return false;
  }
  return waitI3CReady(1000);
}

static bool initISM6HG256X(ImuSlot &imu)
{
  uint8_t who = 0;
  if (!i3cReadReg(imu.dynAddr, REG_WHO_AM_I, &who, 1)) {
    return false;
  }

  VCP.print(imu.label);
  VCP.print(" WHO_AM_I = 0x");
  printHex8(who);
  VCP.println();

  if (who != WHO_EXPECTED) {
    VCP.println("ERR: expected WHO_AM_I 0x73");
    return false;
  }

  if (!i3cWriteReg(imu.dynAddr, REG_CTRL3, 0x44)) {  // BDU + IF_INC
    return false;
  }
  if (!i3cWriteReg(imu.dynAddr, REG_CTRL8, 0x03)) {  // accel FS = +/-16 g
    return false;
  }
  if (!i3cWriteReg(imu.dynAddr, REG_CTRL6, 0x04)) {  // gyro FS = +/-2000 dps
    return false;
  }
  if (!i3cWriteReg(imu.dynAddr, REG_CTRL1, 0x06)) {  // accel ODR = 120 Hz
    return false;
  }
  if (!i3cWriteReg(imu.dynAddr, REG_CTRL2, 0x06)) {  // gyro ODR = 120 Hz
    return false;
  }

  // accel + gyro must be running before enabling SFLP (the engine consumes them internally).
  delay(20);

  if (!enableSflp(imu.dynAddr)) {
    VCP.print(imu.label);
    VCP.println(" SFLP enable failed");
    return false;
  }

  delay(20);
  return true;
}

// Turn on the SFLP game rotation vector. All embedded-bank writes are bracketed by a bank switch,
// and the main bank is ALWAYS restored at the end (even on failure) so the raw OUTX path stays valid.
static bool enableSflp(uint8_t dynAddr)
{
  bool ok = true;
  uint8_t reg = 0;

  ok = ok && i3cWriteReg(dynAddr, REG_FUNC_CFG_ACCESS, 0x80);  // enter embedded-function bank

  if (ok) ok = i3cReadReg(dynAddr, REG_EMB_SFLP_ODR, &reg, 1);
  if (ok) {
    reg = (uint8_t)((reg & ~0x38) | (SFLP_ODR_120HZ_CODE << 3));  // sflp_game_odr = 120 Hz
    ok = i3cWriteReg(dynAddr, REG_EMB_SFLP_ODR, reg);
  }

  if (ok) ok = i3cReadReg(dynAddr, REG_EMB_FUNC_EN_A, &reg, 1);
  if (ok) {
    reg = (uint8_t)(reg | 0x02);  // sflp_game_en
    ok = i3cWriteReg(dynAddr, REG_EMB_FUNC_EN_A, reg);
  }

  if (ok) ok = i3cReadReg(dynAddr, REG_EMB_FUNC_INIT_A, &reg, 1);
  if (ok) {
    reg = (uint8_t)(reg | 0x02);  // sflp_game_init
    ok = i3cWriteReg(dynAddr, REG_EMB_FUNC_INIT_A, reg);
  }

  const bool restored = i3cWriteReg(dynAddr, REG_FUNC_CFG_ACCESS, 0x00);  // back to main bank (always)
  return ok && restored;
}

static bool readImuBurst(uint8_t dynAddr, int16_t &gx, int16_t &gy, int16_t &gz, int16_t &ax, int16_t &ay, int16_t &az)
{
  if (!i3cReadReg(dynAddr, REG_OUTX_L_G, rxBuf, 12)) {
    return false;
  }

  gx = (int16_t)((uint16_t)rxBuf[1] << 8 | rxBuf[0]);
  gy = (int16_t)((uint16_t)rxBuf[3] << 8 | rxBuf[2]);
  gz = (int16_t)((uint16_t)rxBuf[5] << 8 | rxBuf[4]);
  ax = (int16_t)((uint16_t)rxBuf[7] << 8 | rxBuf[6]);
  ay = (int16_t)((uint16_t)rxBuf[9] << 8 | rxBuf[8]);
  az = (int16_t)((uint16_t)rxBuf[11] << 8 | rxBuf[10]);
  return true;
}

// Read the SFLP game rotation vector (W,X,Y,Z half-floats) from the embedded-function bank. Restores
// the main bank before returning so the next raw OUTX read is unaffected.
static bool readSflpQuat(uint8_t dynAddr, float &qw, float &qx, float &qy, float &qz)
{
  if (!i3cWriteReg(dynAddr, REG_FUNC_CFG_ACCESS, 0x80)) {  // enter embedded-function bank
    i3cWriteReg(dynAddr, REG_FUNC_CFG_ACCESS, 0x00);
    return false;
  }
  uint8_t q[8] = {0};
  const bool ok = i3cReadReg(dynAddr, REG_EMB_SFLP_QUATW_L, q, 8);
  const bool restored = i3cWriteReg(dynAddr, REG_FUNC_CFG_ACCESS, 0x00);  // back to main bank (always)
  if (!ok || !restored) {
    return false;
  }
  qw = halfToFloat((uint16_t)((uint16_t)q[1] << 8 | q[0]));
  qx = halfToFloat((uint16_t)((uint16_t)q[3] << 8 | q[2]));
  qy = halfToFloat((uint16_t)((uint16_t)q[5] << 8 | q[4]));
  qz = halfToFloat((uint16_t)((uint16_t)q[7] << 8 | q[6]));
  return true;
}

// IEEE 754 binary16 -> binary32. Quaternion components are in [-1, 1] (normal range), so the
// subnormal branch is essentially never taken; it is included for correctness.
static float halfToFloat(uint16_t h)
{
  const uint32_t sign = (uint32_t)(h & 0x8000) << 16;
  uint32_t exp = (h >> 10) & 0x1F;
  uint32_t mant = h & 0x3FF;
  uint32_t bits;

  if (exp == 0) {
    if (mant == 0) {
      bits = sign;  // +/- zero
    } else {
      int e = -1;
      do {
        e++;
        mant <<= 1;
      } while ((mant & 0x400) == 0);
      mant &= 0x3FF;
      bits = sign | ((uint32_t)(112 - e) << 23) | (mant << 13);
    }
  } else if (exp == 0x1F) {
    bits = sign | 0x7F800000u | (mant << 13);  // inf / nan
  } else {
    bits = sign | ((exp + 112) << 23) | (mant << 13);  // normal: 127 - 15 = 112
  }

  union {
    uint32_t u;
    float f;
  } conv;
  conv.u = bits;
  return conv.f;
}
